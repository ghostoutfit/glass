...
export const THERMAL_SPEED      = 0.0015  // v_rms = THERMAL_SPEED × √T  (px/substep per √°C); 0.3× original for calmer visuals — bond-break calibration deferred to Phase 3
const FREE_ATOM_SPEED_MULT = 0.005 / 0.0015  // freed atoms run at original thermal speed so they visibly drift
...
export function stepPhysics(phys, ePerParticle, coolingFactor = 1.0, attractK = 0, coolingMode = null) {
  ...
  for (let sub = 0; sub < SUBSTEPS; sub++) {
    ...
    if (coolingFactor < 1.0 - 1e-6) {
      ...
      const kickSigmaFree     = kickSigma * FREE_ATOM_SPEED_MULT
      const CRYST_JIGGLE_MULT = 4.0   // SiO2 atoms jiggle at 4× normal — visible thermal vibration in the crystal
      const kickSigmaCryst    = kickSigma * CRYST_JIGGLE_MULT
      const prevIntact        = phys.intactCount  // from previous step; null on first step
      for (let i = 0; i < n; i++) {
        const ks = (prevIntact && prevIntact[i] === 0) ? kickSigmaFree
                 : crystAnchor[i] || naAnchor[i]        ? kickSigmaCryst
                 : kickSigma
        const u1 = Math.random() || 1e-10
        const r  = Math.sqrt(-2 * Math.log(u1)) * ks  // Box-Muller magnitude
